...
**generation 层（LLM 调用细节）用 `start_as_current_observation`**：在 [llm/gateway.py](../../src/finance_agent/llm/gateway.py) 的 `complete_text` / `complete_stream` / `complete_with_tools` 三个入口内（旧路径 `llm.py` 的 `call_llm` / `call_llm_stream` / `call_llm_with_tools`），用 `with langfuse.start_as_current_observation(as_type="generation", name=<model>, model=<model>, input=...) as gen:` 包裹 `litellm.completion` 调用，结束后 `gen.update(output=, usage_details=)`。因为这三函数在节点内运行、节点已被 CallbackHandler 挂在正确父级下，generation 自动归到对应 Agent span。Generation 附带 `prompt_name` + `prompt_version`（见 ADR-0016）。
...
- **新增耦合**：`complete_text` / `complete_stream` / `complete_with_tools` 三入口对 Langfuse SDK 的依赖。但 Langfuse 仍可选（未配置 key 时跳过观测，与现状一致）。
...
- [llm/gateway.py](../../src/finance_agent/llm/gateway.py) - 待删除的坏集成代码
