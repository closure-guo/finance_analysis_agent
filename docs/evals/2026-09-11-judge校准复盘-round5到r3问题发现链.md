...
## 9. citation 门禁整改方案（2026-09-11 与 owner 共同定稿）

详见 `docs/incidents/026-citation-gate-misattribution-20260911.md`。要点：

- **指标拆三报**：分析师真幻觉率 0/565（抽样归因，待逐条终裁）；校验器误报率 39/467；结构不可验占比 98/565≈17%。混报会让评审以为系统有四分之一引用问题，实际为零。
- **修复按杠杆排序**：第 0 步停掉误报桶上的自动重试并加「重写文本不变即停」（配置级，当天）；第 1 步校验器四类归一（清零后做注入式故障演练验证门禁仍会开火）；第 2 步计算型字段注册重算；第 3 步文本 claim 用既有 claim_type/source_type 分流 + 回声匹配（NLI 备选）；第 4 步 auto-claim 补覆盖缺口。
- **门禁三层分置**：阻断 = 真 FAIL（终裁后）=0；警告 = coverage<0.90；跟踪 = UNVERIFIABLE 占比。需 delta 修订 citation 相关规范，`citation_minor_fail` 随之退役。
- **纪律**：处置对象必须匹配归因桶——契约病修契约、解析病修解析器、真幻觉才修分析师。
