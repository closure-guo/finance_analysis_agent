...
**观测电池第 1 轮（2026-09-22，delta `add-causal-observation-battery` 首跑，HEAD ec3122b）**：固定 P1 快照 × v28 生产栈（20 标的全量材料重跑 + 三判定）。读数：**grounding 无源率 0.049（4/82）**、**B1 吸收率 0.848（212/250）**、**B2 修正率 0.082（122/1,487，全量口径首读）**；判定调用 **1,820 次**（B2 全量占 1,487），材料腿 usage meter 未接线（导入修复已随 ec3122b 落地，本轮如实记 unknown）。对照第 0 轮（09-20）：B1 0.833→0.848（同口径 +1.5pp）；**grounding 0/79→4/82 出现回退，待分桶归因**（观测不裁决，4 条无源断言清单在产物 JSON，处置候选走 owner）；B2 跨口径不可比（0.138 为 80 行闸门口径）。产物 `reports/ablation/observation/observation-20260922-225708.json`（本地）+ `snapshot-digests.json`（下轮核验基准）。
