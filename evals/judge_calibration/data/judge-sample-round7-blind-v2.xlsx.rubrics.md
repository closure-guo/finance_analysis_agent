...
### 全量审计后的补充（2026-09-11，41 行逐行审过）

- **用 v2 文件**：`judge-sample-round7-blind-v2.xlsx`（同一批 trace、同一套 judge 分，仅 9 行 consistency 的【Risk Judge 裁决】节多了一行「【附：裁决所依据的风险辩论】」边界标签——标签之后是风险辩论原文，不是裁决内容）。如已在 v1 上开标，继续用 v1 即可，标签之前/之后的信息完全相同。
- **已知截断 1 行**：decision_grounding、trace `d7e48a92…`（贵州茅台）的【风险辩论记录】激进方第二轮正文中段有「...[truncated 17899 bytes]...」——judge 当时看到的也是这样（人机对等），评分时对激进方论点完整性打折并压 confidence，不必停。
- **以下是评分对象，不是材料错误**：FM 头部「approve（sell，置信度 0.55）」与其理由里的 0.52 不一致、FM 置信度 0.6 疑似照抄 RM 的 0.60——这类跨层置信度矛盾正是 consistency 该扣的；quick 报告里「你的总结很准确，我再补充两点」这种自说自话属 report_relevance 评分范围。
- **系统字段名词**：`entry/stop/target 为 null` = 方案没给入场价/止损/目标价；`moderate/light` = 仓位档；「退回额度用尽」= FM 最多退回 Trader 一次。看到这些按字面理解即可。
