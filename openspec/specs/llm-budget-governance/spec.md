# llm-budget-governance Specification

## Purpose

定义 LLM 预算治理契约：上下文总预算从当前 `ModelProfile.capability.max_context` 派生（禁止全局固定值），输出预算与观测预算字段（max_tokens 派生来源、usage_estimated、error_type）在每次 generation 观测中强制携带，保证预算决策可审计、可校准。
