...
class LiteLLMClient:
    ...
    def _build_kwargs(
        ...
        # DeepSeek: thinking mode 与 tool calling 不兼容
        ...
        elif is_ds and thinking == "enabled":
    ...
    async def chat_stream(
        ...
        if self._langfuse:
            try:
                _lf_obs = self._langfuse.start_observation(
                    name=f"litellm:{self.model}",
                    as_type="generation",
                    input={"messages": messages},
                    model=self.model,
                    completion_start_time=_start_time,
                )
            except Exception as e:
            ...
            try:
                ...
                async for chunk in response:
                    ...
                    if finish_reason == "tool_calls" and current_tool_calls:
                        ...
                        self._finish_langfuse(_lf_obs, _accumulated_text, chunk)
                ...
                if current_tool_calls:
                    parsed = self._parse_tool_calls(current_tool_calls)
                    self._finish_langfuse(_lf_obs, _accumulated_text, None)
                    yield LLMResponse(tool_calls=parsed, is_finished=True)
                else:
                    self._finish_langfuse(_lf_obs, _accumulated_text, None)
                    yield LLMResponse(is_finished=True)
    ...
    def _finish_langfuse(self, obs, text: str, last_chunk) -> None:
        """流结束后更新 Langfuse 观测。"""
        if not obs:
            return
        try:
