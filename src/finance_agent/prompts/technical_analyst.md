...
6.5. 数值型/计算型 claim 还须申报 direction（positive/negative/flat）：direction 修饰 stated_value 的符号语义——正文写「下滑 X%」而真值为负 → stated_value=X、direction=negative；正文直接写 signed 值（如 -X%）→ direction=positive；存量水平类（MA/RSI 等指标水平）无数值方向语义 → flat。缺 direction 计覆盖缺口；校验器按 sign(stated_value)×direction 对齐真值符号，申报方向与真值符号冲突判 direction_mismatch 打回
