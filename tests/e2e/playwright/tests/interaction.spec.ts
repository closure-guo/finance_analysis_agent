 ...
 *   2) 点击 "快速模式" 选项切换 mode
 *
 * 另：按 plan 注意事项，不直接断言按钮 disabled（快速模式的发送按钮在流式中可能未
 * 实现 disabled）。改为断言 stream-status 的可见性周期（出现 -> 消失）作为交互状态验证。
 * 关键 timeout 放宽到 15_000，与 streaming.spec 一致，覆盖 ReAct Agent 初始化 + SSE 传输。
 */
test('发送中流式指示器可见性周期（出现 -> 消失）', async ({ page }) => {
  ...
  // 验证流式指示器出现（证明正在流式中）
  await expect(page.getByTestId('stream-status')).toBeVisible({ timeout: 15_000 })
