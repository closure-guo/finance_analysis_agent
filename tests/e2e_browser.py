...
def _mock_llm(prompt: str, system: str = "", temperature: float = 0.3, max_tokens: int = 4096) -> str:
...
def _mock_fetch_data(state: dict, cache=None, client=None) -> dict:
    ...
    return {
        ...
        "stock_quote": {"name": "贵州茅台", "code": code, "PE": 25.0, "PB": 8.0, "price": 1700.0, "market_cap": 21000.0},
    ...
    try:
        with sync_playwright() as p:
            ...
            for _ in range(60):
                ...
                for selector in [".prose", ".markdown", "[data-testid='markdown']", ".gradio-markdown", "[class*='md']"]:
                    try:
            ...
            checks = {
                ...
                "包含分析相关文字": "分析" in report_text or "茅台" in report_text or "股票" in report_text,
            }
            ...
            for txt in ["下载", "Word", "PPT", ".docx", ".pptx"]:
                try:
                    btns = page.locator("a, button, [class*='download'], [class*='file']").filter(has_text=txt)
                    if btns.count() > 0:
                        has_download = True
                        break
                except Exception:
