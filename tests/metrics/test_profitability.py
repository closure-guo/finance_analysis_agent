...
class TestCalcProfitability:
    ...
    def test_zero_revenue(self):
        ...
        bs = pd.DataFrame(
            {"报告日": ["20241231"], "所有者权益(或股东权益)合计": [500.0],
             "短期借款": [0.0], "长期借款": [0.0], "应付债券": [0.0],
             "一年内到期的非流动负债": [0.0]}
        )
        is_ = pd.DataFrame(
            {"报告日": ["20241231"], "营业收入": [0.0], "营业成本": [0.0],
             "净利润": [0.0], "利润总额": [0.0], "所得税费用": [0.0], "利息费用": [0.0]}
        )
        ind = pd.DataFrame(
            {"日期": ["2024-12-31"], "销售毛利率(%)": [None], "销售净利率(%)": [None],
             "净资产收益率(%)": [None], "总资产净利润率(%)": [None]}
        )
