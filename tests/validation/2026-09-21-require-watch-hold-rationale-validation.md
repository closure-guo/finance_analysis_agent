...
既有测试更新（行为变更的对象，非改弱）：`test_risk.py::test_watch_no_price_requirement`（watch mock 补理由字段）、`test_report.py::test_watch_no_price_rows_and_trigger_hint` → `test_watch_missing_rationale_marked_unprovided`（旧占位行「见理由」断言的替换对象）。
...
5. 浏览器 E2E 未跑：非交互类变更（判别见 §1），且无前端消费该文本（前端读结构化决策字段而非报告 markdown，Task 5 审查已核）。
6. judge 材料自动携带新字段（`evals/extract.py::_serialize_decision`）——B5 可执行性维度的打分基线可能移动，属预期增益；按「同噪声下相对差异」解读，不追历史绝对分。
