# citation 22 条待终裁决策单（✅ 终裁完成 2026-09-13）

> **终裁结论（维护者代裁，证据链齐备）**：22 条全部「非幻觉」。依据：①13 行
> r4 同族全 PASS（新规则吸收）；②8 行混合族的残余全部落在两个已知「结构不可验」
> 形态——比较/解读句（组件数字全 PASS，差值可机械推导）与 quarterly_trend 期次
> 路径（bucket 统一 path_unresolvable，无「值不匹配」型失败），其中美的 1.32% 已
> 与 round7 材料 1.3245 实锤对账；③r4-1/r4-2 为列名单位后缀缺口，已修复
> （metric_vocab 补「加权每股收益(元)」等别名 + _resolve_field_ref 列名别名回退 +
> 路径止于列名时取最新一行）。
> **Follow-up 登记（metrics.md）**：比较型 claim 重算注册；quarterly_trend 期段定位。
