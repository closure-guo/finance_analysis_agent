...
| trace 记录会话内容（根 span output） | 管线根 span（`deep_analysis:{股票}`）与 ReAct `react_loop` span 退出前将 agent 产出写入 span output，使 Langfuse trace/session 层级直接可见 agent 输出而非仅有 user input | ✅ `tests/test_trace_output.py::test_build_trace_output_summarizes_agent_outputs`（`_build_trace_output` 从 accumulated 构建摘要：stock/最终报告摘要截断≤600 字符/analyst 摘要/决策字段）、`test_stream_graph_exposes_root_obs_via_sink`（`_stream_graph` 经 `root_obs_sink` 透传根 obs 句柄供事件循环侧写 output）；`tests/test_deep_trace_root.py`（root span 建立/session 传播/无 Langfuse 不崩——根 span 生命周期）；`tests/test_sse_stream.py::TestStreamAgentToSse::test_react_loop_span_writes_answer_output`（直接驱动 `stream_agent_to_sse` 到退出路径，断言 react_loop obs 的 `update(output={"answer": 累积 ANSWER 内容})` 被执行）与 `test_react_loop_span_skips_output_when_no_answer`（空答案守卫：无 ANSWER 事件时不写 output）。注：`deep_analysis` 根 span 的 output 写入路径（`_background_consume` 完成点）仅由 post-merge 实跑对账覆盖（下方风险说明） | ⬜ 待人工（post-merge Langfuse UI）：`deep_analysis:{股票}` 根 span 的 **output 非 null**，含管线产出摘要（stock_code/stock_name/final_report_summary/analyst_reports/决策字段）；`react_loop` span 的 **output 含 `answer`**（agent 最终回复）。对账步骤见下方「实跑对账说明」第 5 项 |

> **风险说明**：`deep_analysis` 根 span 的 output 更新发生在根 span 退出之后（`_background_consume` 在 `_stream_graph` 的根 span 已 `__exit__` 后才 `_root_obs.update(...)`），该「退出后更新 + flush 时序」是否在 Langfuse v4 真实生效仅能由 post-merge 实跑对账验证（delta tasks.md 4.1）；单测只验证 update 调用本身，不验证 Langfuse 侧时序。
